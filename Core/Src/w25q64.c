...
HAL_StatusTypeDef W25Q64_OCTO_SPI_Init(OSPI_HandleTypeDef *hospi)
{
    ...
    if (W25Q64_OSPI_ResetChip(hospi) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (W25Q64_OSPI_Configuration(hospi) != HAL_OK)
    {
        return HAL_ERROR;
    }
    HAL_Delay(1);
    ...
    if (W25Q64_OSPI_WriteEnable(hospi) != HAL_OK)
    {
        return HAL_ERROR;
    }
    return HAL_OK;
...
/* Reset Chip Function */
HAL_StatusTypeDef W25Q64_OSPI_ResetChip(OSPI_HandleTypeDef *hospi)
{
...
HAL_StatusTypeDef W25Q64_OSPI_Configuration(OSPI_HandleTypeDef *hospi)
{
    uint8_t reg1 = 0, reg2 = 0, reg3 = 0;
    uint8_t w_reg1 = 0, w_reg2 = 0, w_reg3 = 0;

    if (W25Q64_Read_Status_Registers(hospi, &reg1, 1) != HAL_OK)
    {
        return HAL_ERROR;
    }

    if (W25Q64_Read_Status_Registers(hospi, &reg2, 2) != HAL_OK)
    {
        return HAL_ERROR;
    }

    if (W25Q64_Read_Status_Registers(hospi, &reg3, 3) != HAL_OK)
    {
        return HAL_ERROR;
    }
    ...
    if (W25Q64_Write_Status_Registers(hospi, w_reg1, 1) != HAL_OK)
    {
        return HAL_ERROR;
    }
    if (W25Q64_Write_Status_Registers(hospi, w_reg2, 2) != HAL_OK)
    {
        return HAL_ERROR;
    }

    if (W25Q64_Write_Status_Registers(hospi, w_reg3, 3) != HAL_OK)
    {
        return HAL_ERROR;
    }
...
HAL_StatusTypeDef W25Q64_OSPI_WriteEnable(OSPI_HandleTypeDef *hospi)
{
...
HAL_StatusTypeDef W25Q64_OSPI_AutoPollingMemReady(OSPI_HandleTypeDef *hospi)
{
...
/* Erase Chip Function */
HAL_StatusTypeDef W25Q64_OSPI_Erase_Chip(OSPI_HandleTypeDef *hospi)
{
    OSPI_RegularCmdTypeDef sCommand = {0};
...
HAL_StatusTypeDef W25Q64_OSPI_EraseBlock(OSPI_HandleTypeDef *hospi, uint32_t EraseStartAddress, uint32_t EraseEndAddress)
{
    OSPI_RegularCmdTypeDef sCommand = {0};
    uint32_t StartAddress = 0;

    StartAddress = EraseStartAddress - (EraseStartAddress % W25Q_BLOCK_SIZE);
    ...
    while (EraseEndAddress >= StartAddress)
    {
        ...
        StartAddress += W25Q_BLOCK_SIZE;
    }

    return HAL_OK;
}

HAL_StatusTypeDef W25Q64_OSPI_EraseSector(OSPI_HandleTypeDef *hospi, uint32_t EraseStartAddress, uint32_t EraseEndAddress)
{
    OSPI_RegularCmdTypeDef sCommand = {0};
    uint32_t StartAddress = EraseStartAddress - (EraseStartAddress % W25Q_SECTOR_SIZE);
    ...
    while (EraseEndAddress >= StartAddress)
    {
        ...
        sCommand.Instruction = W25Q_SECTOR_ERASE_CMD; // 0x20
        sCommand.AddressMode = HAL_OSPI_ADDRESS_1_LINE;
        sCommand.AddressSize = HAL_OSPI_ADDRESS_24_BITS;
        sCommand.Address = (StartAddress & 0xFFFFFF);
...
/* Write Function */
HAL_StatusTypeDef W25Q64_OSPI_Write(OSPI_HandleTypeDef *hospi, uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
    OSPI_RegularCmdTypeDef sCommand = {0};
    ...
    while (current_addr < end_addr)
    {
        ...
        // memset(temp_buffer, 0xFF, Size); // Fill buffer with erased state
        // memcpy(temp_buffer, pData, Size);  // Copy data to temp buffer
        ...
        sCommand.InstructionMode = HAL_OSPI_INSTRUCTION_1_LINE;
        sCommand.InstructionSize = HAL_OSPI_INSTRUCTION_8_BITS;
        sCommand.Instruction = W25Q_PAGE_PROGRAM_QUAD_INP_CMD; // 0x32
        sCommand.AddressMode = HAL_OSPI_ADDRESS_1_LINE;
        sCommand.AddressSize = HAL_OSPI_ADDRESS_24_BITS;
...
/* Read Function */
HAL_StatusTypeDef W25Q64_OSPI_Read(OSPI_HandleTypeDef *hospi, uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
    OSPI_RegularCmdTypeDef sCommand = {0};
    ...
    sCommand.DQSMode = HAL_OSPI_DQS_DISABLE;                               /* Disable Data Strobe */
    sCommand.SIOOMode = HAL_OSPI_SIOO_INST_EVERY_CMD;                      /* SIOO Mode: Send instruction on every transaction */
    sCommand.AlternateBytesMode = HAL_OSPI_ALTERNATE_BYTES_4_LINES;           /* Disable Alternate Bytes Mode */
    sCommand.AlternateBytes = 0xff; //HAL_OSPI_ALTERNATE_BYTES_NONE;               /* Alternate Bytes = 0 */
    sCommand.AlternateBytesSize = HAL_OSPI_ALTERNATE_BYTES_8_BITS;           /* Alternate Bytes Size = 0 */
    sCommand.AlternateBytesDtrMode = HAL_OSPI_ALTERNATE_BYTES_DTR_DISABLE; /* Disable Alternate Bytes DDR/DTR Mode */
    sCommand.InstructionMode = HAL_OSPI_INSTRUCTION_1_LINE;                /* Instruction on a single line */
    sCommand.InstructionSize = HAL_OSPI_INSTRUCTION_8_BITS;                /* 8-bit Instruction */
    sCommand.AddressSize = HAL_OSPI_ADDRESS_24_BITS;                       /* 24-bit Address */
    ...
    sCommand.DummyCycles = 4; //W25Q_DUMMY_CYCLES_READ_QUAD; /* Bytes Send With No Data */
...
HAL_StatusTypeDef W25Q64_OSPI_EnableMemoryMappedMode(OSPI_HandleTypeDef *hospi)
{
...
HAL_StatusTypeDef W25Q64_IsBusy(OSPI_HandleTypeDef *hospi)
{
    HAL_StatusTypeDef state;
    uint8_t status_rgister = {0};

    state = W25Q64_Read_Status_Registers(hospi, &status_rgister, 1);
    if (state != HAL_OK)
        return state;
...
HAL_StatusTypeDef W25Q64_Read_Status_Registers(OSPI_HandleTypeDef *hospi, uint8_t *register_data, uint8_t register_num)
{
    OSPI_RegularCmdTypeDef sCommand = {0};
...
HAL_StatusTypeDef W25Q64_Write_Status_Registers(OSPI_HandleTypeDef *hospi, uint8_t reg_data, uint8_t reg_num)
{
    OSPI_RegularCmdTypeDef sCommand;
